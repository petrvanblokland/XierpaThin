...
class A(object):
    ...
    @classmethod
    def fromJson(cls, s):
        return cls(cjson.decode(s)) # Always answer a dictionary --> ADict.
...
class AList(A):
    ...
    def asJson(self):
        return cjson.encode(dict(records=self.asList()))
    ...
    def asList(self):
        u"""Return the contents as a list.
            ...
            >>> tl.asList()
            [1, 2, 3]
            >>> tl = AList([1, 2, AList([3, 4])])
            >>> tl.asList()
            [1, 2, [3, 4]]
        """
        l = []
        for element in self._data_:
            if isinstance(element, AList):
                l.append(element.asList())
            elif isinstance(element, ADict):
                l.append(element.asDict())
            else:
                l.append(element)
        return l
...
class ADict(A):
    ...
    def asDict(self):
        u"""Answer the internal dictionary.
            ...
            >>> td.asDict()
            {'a': 1, 'b': [2, 3]}
            >>> td = ADict(dict(a=1, b=ADict(dict(c=4, d=AList((6,7,8))))))
            >>> td.asDict()
            {'a': 1, 'b': {'c': 4, 'd': [6, 7, 8]}}
        """
        d = {}
        for key, element in self._data_.items():
            if isinstance(element, AList):
                d[key] = element.asList()
            elif isinstance(element, ADict):
                d[key] = element.asDict()
            else:
                d[key] = element
        return d
    ...
    def asJson(self):
        u"""
            >>> td = ADict(dict(a=1, b=2, c=3))
            >>> td.asJson()
            '{"a": 1, "c": 3, "b": 2}'
            >>> ADict.fromJson(td.asJson()).asDict()
            {'a': 1, 'c': 3, 'b': 2}
        """
        return cjson.encode(self.asDict())
